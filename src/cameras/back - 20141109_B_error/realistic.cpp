	...
	film(f)
{
	...
	// for perspective
	// - GetTransformData(f, totallength, fdag, hither, yon, RasterToCamera);
	//Calculate effective camera film resolutions
	float aspectRatio = film->xResolution / film->yResolution;
	yCamFilmRes = sqrt((fdag*fdag) / (1 + (aspectRatio*aspectRatio)));
...
//
//	// - cameratoscreen
//	Transform cameratoscreen = Scale(1.f, 1.f, 1.f / (yon - hither)) * Translate(Vector(0.f, 0.f, totallenth));
//
//	// - screentoraster
//	Transform screentoraster = Scale(float(f->xResolution), float(f->yResolution), 1.f) *
